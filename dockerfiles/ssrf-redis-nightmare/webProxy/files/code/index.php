...
$p_request_url['scheme'] = strtolower($p_request_url['scheme']);
$p_request_url['path'] = strtolower($p_request_url['path']);

//WAF
$service_index = file_get_contents('http://rancher-metadata/latest/self/container/service_index');

$redis = dns_get_record('redis', DNS_A);
$redis_ips = [];
foreach ($redis as $d) {
  $redis_ips[] = $d['ip'];
}

if(in_array($p_request_url['host'], $redis_ips) || strpos($request_url, '/var/www/html') !== false
|| strpos($request_url, 'save') !== false || strpos($request_url, 'savebg') !== false
 || strpos($request_url, 'config') !== false || strpos($request_url, 'set') !== false
|| strpos($p_request_url['scheme'], 'gopher') !== false) {
  echo "BLOCKED BY WAF!";
  return;
}
